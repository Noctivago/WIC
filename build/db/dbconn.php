...
function alteraFirst($pdo) {
    try {
        $rows = sql($pdo, "Select * From [Service] where [Id] >= ?", array(10513), "rows");
        foreach ($rows as $row) {
            $serv = $row['Id'];
...
Function DB_getMyWicsAsPopup($pdo, $userId) {
    try {
        ...
        if (DB_checkIfUserHaveWicPlanner($pdo, $userId)) {
            echo '<div class="title-label" style="align:center;"> <input type=button onclink="redirect();" class="btn btn-rounded btn-inline btn-primary" href ><i class="font-icon font-icon-plus "></i>&ensp;New Event></div>
