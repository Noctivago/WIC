...
function alteraFirst($pdo) {
    try {
        $rows = sql($pdo, "Select * From [Service] where [Id] >= ?", array(8000), "rows");
        foreach ($rows as $row) {
            $serv = $row['Id'];
...
//        alterações feitas 11/08/2016 mudar button to input e colocar type="button"
        if (DB_checkIfUserHaveWicPlanner($pdo, $userId)) {
            echo '<div class="title-label" style="align:center;">  <input type="button" class="btn btn-rounded btn-inline btn-primary" href ><i class="font-icon font-icon-plus "></i>&ensp;New Event?</div>
                <h6>WiC Planner - The notepad for event planners</h6>
                <header class="sign-title">Address to an existent one</header>';
            
            //        Fim alterações feitas 11/08/2016
